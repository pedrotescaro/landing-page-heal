...
function getFormData() {
    ...
    [
        "ferida_largura", "ferida_comprimento", "ferida_profundidade",
        "dor_escala", "percentual_granulacao_leito", "percentual_epitelizacao_leito",
...
form.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        window.location.href = "dashboard.html?messageType=success&messageText=" + encodeURIComponent(`Anamnese ${editingAnamneseId ? "atualizada" : "criada"} com sucesso!`);
...
async function loadAnamneseData(id) {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || "Erro ao carregar anamnese para edição.");
        }
        ...
        window.location.href = "dashboard.html?messageType=error&messageText=" + encodeURIComponent(`Não foi possível carregar a anamnese para edição: ${error.message}`);
    }
}
...
function loadAnamnesesForList() {
    ...
    fetch(API_ENDPOINTS.anamneses)
        ...
        .then((data) => {
            ...
            if (data.length === 0) {
                listContainer.innerHTML = '<p style="text-align: center; color: #888;">Nenhuma anamnese cadastrada ainda.</p>';
                return;
            }
            ...
            data.forEach((anamnese) => {
                ...
                li.innerHTML = `
                    <strong>${anamnese.nome_cliente || 'Paciente sem nome'}</strong> (ID: ${anamnese.id})<br>
                    <button class="edit-anamnese-btn" data-id="${anamnese.id}">Editar</button>
                    <button class="delete-anamnese-btn" data-id="${anamnese.id}">Excluir</button>
                `;
        ...
        .catch((error) => {
            console.error("Erro ao carregar anamneses para a lista de finalização:", error);
...
function populateFormWithData(data) {
    ...
    for (const parentId in conditionalDisplayMap) {
        ...
        } else if (parentElement.type === "radio") { // Se você tiver radios
            shouldShow = parentElement.checked;
        }
        ...
        childrenElements.forEach((child) => {
            ...
            if (!shouldShow && (child.tagName === "INPUT" || child.tagName === "TEXTAREA" || child.tagName === "SELECT")) {
                 if (child.type !== "checkbox" && child.type !== "radio") { // Evita limpar checkboxes/radios
                    child.value = "";
                 }
            }
        });
...
async function deleteAnamnese(id) {
    if (!confirm("Tem certeza que deseja excluir esta anamnese? Esta ação é irreversível.")) {
        return;
    }
    try {
        const response = await fetch(API_ENDPOINTS.anamneseById(id), {
